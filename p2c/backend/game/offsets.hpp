...
struct o
{
	...
	uintptr_t gameinstance = 0x1d0; //struct UGameInstance* OwningGameInstance;
	...
	uintptr_t relativelocation = 0x120; //struct FVector RelativeLocation;
	uintptr_t mesh= 0x318; //struct USkeletalMeshComponent* Mesh;
	uintptr_t CurrentActor = 0x8; //??
	uintptr_t teamid = 0x10f1; //char TeamIndex; (AFortPlayerStateAthena : AFortPlayerStateZone)
	...
	uintptr_t comptowrld = 0x230; //struct TArray<struct FBPComponentClassOverride> ComponentClassOverrides;

	uintptr_t bonearray = 0x608; //??
	uintptr_t bonec = 0x658;// ??

	uintptr_t namepoolchunk = 0x10EFDA00;

	uintptr_t lastsubmit = 0x358; //??
	uintptr_t lastrender = 0x360; //??
