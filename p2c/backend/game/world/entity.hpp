	...
	public:
		...
		auto loop() -> void
		{
			...
			if (misc->watermark)
			{
				if (misc->show_fps) {
					char Watermark[64];
					sprintf(Watermark, _("[%.1f FPS]\n"), ImGui::GetIO().Framerate);
					float fontSize = 18;
					float textWidth = ImGui::CalcTextSize(_("")).x * (fontSize / ImGui::GetFontSize());
			...
			for (uint32_t i = 0; i < cached->entitycount; i++)
			{
				...
				auto CurrentWeapon = driver.read<uintptr_t>(CurrentActor + 0xa20);
				if (!CurrentActor) 
				{
					continue;
				}
				...
				Vector3 Headpos = uengine->bonewithrotation(mesh, 109);
