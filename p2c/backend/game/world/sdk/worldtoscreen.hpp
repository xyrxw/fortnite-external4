...
struct unrealengine
{
	...
	{
		SPOOF;
		uintptr_t bone_array = driver.read<uintptr_t>(mesh + 0x598);
		if (bone_array == NULL) bone_array = driver.read<uintptr_t>(mesh + 0x598 + 0x10);
		FTransform bone = driver.read<FTransform>(bone_array + (bone_id * 0x60));
		FTransform component_to_world = driver.read<FTransform>(mesh + 0x1c0);
